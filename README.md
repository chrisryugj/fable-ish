...
fable-ish는 위험한 셸 명령이나 비밀 파일 편집을 **차단하지 않습니다** — 일부러 그렇게 설계했습니다.

정규식으로 셸의 *의도*를 판정하는 건 지는 싸움입니다. 명령을 정규식으로 막는 룰셋을 실측해보면 위험 명령 16개 중 **4개만** 막힙니다 — `rm -r -f`(플래그 분리), `rm --recursive --force`, `$VAR -rf`, base64 파이프 `eval`, `git -C . push`, 그리고 진짜 파괴적인 `dd`·`mkfs`·fork bomb이 전부 빠져나가고, 반대로 빌드 캐시 정리나 릴리스 `npm publish` 같은 정상 작업은 **과차단**됩니다. 변수 확장·따옴표·glob·인코딩이 표면 패턴을 무력화하므로, 정규식을 조여봐야 오탐만 늘어납니다.
